// Monitoring avancé pour SmartManager - Version corrigée
interface LogEntry {
  timestamp: string
...
export const logApiMiddleware = (handler: any, route: string) => {
  return async (req: any, res: any, ...args: any[]) => {
    ...
    res.send = function(data: any) {
      const duration = Date.now() - startTime
      monitoring.recordRequest(route, duration, res.statusCode)
      originalSend.call(this, data)
    }
